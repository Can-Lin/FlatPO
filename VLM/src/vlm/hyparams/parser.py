...
def _parse_cli_overrides(cli_args: List[str]) -> Dict[str, Any]:
    ...
    while idx < len(cli_args):
        ...
        if not token.startswith("--"):
            raise ValueError(
                f"Unexpected extra argument `{token}`. Extra overrides must use `--key value` format."
            )

        if "=" in token:
            key, raw_value = token[2:].split("=", maxsplit=1)
            idx += 1
        else:
            key = token[2:]
            if idx + 1 < len(cli_args) and not cli_args[idx + 1].startswith("--"):
                raw_value = cli_args[idx + 1]
                idx += 2
            else:
                raw_value = "true"
                idx += 1

        overrides[key.replace("-", "_")] = yaml.safe_load(raw_value)

    return overrides


def _load_args_from_file(path: Union[str, Path]) -> Dict[str, Any]:
