...
class FinetuningArguments(
    ...
    use_sam: bool = field(
        default=False,
        metadata={"help": "Whether or not to use SAM-style LoRA finetuning."},
    )
    sam_rho: float = field(
        default=0.05,
        metadata={"help": "SAM neighborhood size."},
    )
    sam_mode: Literal["sam", "flatpo"] = field(
        default="flatpo",
        metadata={
            "help": "SAM variant. `sam` perturbs all selected LoRA params, `flatpo` perturbs top-k% selected params."
        },
    )
    sam_topk_ratio: float = field(
        default=0.05,
        metadata={
            "help": "Top-k ratio used in `flatpo` mode. Must be in (0, 1]."
        },
    )
    sam_peft_weight: Literal["all", "q", "v", "k"] = field(
        default="all",
        metadata={
            "help": (
                "Which LoRA weights receive SAM perturbation. "
                "Use all, q, v, or k."
            )
        },
    )
    sam_random_weight: bool = field(
        default=False,
        metadata={"help": "Whether or not to sample perturbed LoRA weights randomly."},
    )
    sam_adaptive: bool = field(
        default=False,
        metadata={"help": "Whether or not to use adaptive SAM."},
    )
    sam_debug: bool = field(
        default=False,
        metadata={
            "help": "Whether or not to log detailed SAM/FlatPO perturbation and update diagnostics."
        },
    )
    sam_debug_log_steps: int = field(
        default=10,
        metadata={"help": "Log SAM diagnostics every N training steps."},
    )
    ...
    def __post_init__(self):
        ...
        if self.use_sam and self.finetuning_type != "lora":
            raise ValueError("`use_sam` is only valid for LoRA training.")

        if self.use_sam and self.stage != "sft":
            raise ValueError("`use_sam` is only implemented for SFT training.")

        if self.use_sam and (self.sam_topk_ratio <= 0.0 or self.sam_topk_ratio > 1.0):
            raise ValueError("`sam_topk_ratio` must be in (0, 1].")

        if self.use_sam and self.sam_debug_log_steps <= 0:
            raise ValueError("`sam_debug_log_steps` must be positive.")
