    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if args.trainer != "none":
                ...
                baseline_params = [p.clone().detach() for p in framework_before.model.parameters()]
                results = []
                # 3. 遍历 alpha
                for alpha in [i * 0.1 for i in range(-10, 11)]:
                #for alpha in [i * 0.5 for i in range(-2, 3)]:  # -1到1，步长0.5
                    # 用 baseline_params 和 framework_after 的参数进行插值
                    with torch.no_grad():
                        for p_before, p_baseline, p_after in zip(framework_before.model.parameters(),
                                                                baseline_params,
                                                                framework_after.model.parameters()):
                            p_before.data.copy_(alpha * p_baseline + (1 - alpha) * p_after.data)
                    ...
                    # 如果想要恢复回基准模型参数，可以再执行一次拷贝
                    # 这样下一次循环时，如果还想从原始 baseline 开始，就可以继续使用
                    # 否则也可以直接让下一次循环再次从 baseline_params 做拷贝
                    with torch.no_grad():
                        for p_before, p_baseline in zip(framework_before.model.parameters(), baseline_params):
                            p_before.data.copy_(p_baseline)
            ...
            print("Metrics have been saved to metrics_results.xlsx")
            logger.info(metrics)
            wandb.log(metrics)
