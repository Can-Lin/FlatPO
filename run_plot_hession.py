...
class Framework:
    ...
    def train_me(self, train_samples):
        #logger.info(f"Eval sample length is {len(eval_samples)}")
        # Set tokenizer to left padding (so that all the options are right aligned)
        self.tokenizer.padding_side = "left"
...
def main():
    ...
    for train_set_id, train_samples in enumerate(train_sets):
        ...
        print(type(train_samples))  # 打印数据类型
        print(len(train_samples))   # 打印样本数量
        print(train_samples[0])  # 打印第一个样本
        train_dataset = framework_after.train_me(train_samples)
        print(type(train_dataset))  # 打印数据类型
        print(len(train_dataset))   # 打印样本数量
        print(train_dataset[0])  # 打印第一个样本

        framework_after.model.load_state_dict(torch.load(learnt_model))
        framework_after.model.eval()  # 切换到评估模式 
        hessian_dataloader = []
        ...
        # # 调用 PyHessian 进行 Hessian 计算
        # hessian_comp = hessian(
        #     model=framework_after.model,
        #     criterion=loss_fct,
        #     dataloader=hessian_dataloader,  # 传递数据加载器
        #     cuda=True  # 根据需要使用 GPU
        # )
