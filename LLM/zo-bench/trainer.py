...
#from transformers.utils import is_torch_version
...
#from sam_new_quick import SAM
from GAM.gam.gam import GAM
#_is_native_cpu_amp_available = is_torch_greater_or_equal_than_1_10
#_is_native_cpu_amp_available = is_torch_version(">=", "1.10")
...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        delay_optimizer_creation = (
            (
                # 检查分片DDP的通用方式（兼容新旧版本）
                getattr(self, "sharded_ddp", None) not in [None, "simple"]  # 新版本用字符串
                or is_sagemaker_mp_enabled()
            )
            or (
                # 检查FSDP的通用方式
                getattr(self, "fsdp", None) is not None
                or getattr(self.args, "fsdp", None) is not None
            )
            # 如果启用了DeepSpeed也需考虑
        ...
        if args.trainer == "zo_adam":
           ...
           #     self.optimizer = SGD(self.model.parameters(), lr=args.learning_rate, momentum=args.momentum)
            # 1. 首先确保 SAM 需要的配置在 args 中
            #assert args.lr_scheduler_type == 'constant', "we did not implement lr_schedule."
            if args.optimizer == "adam":
                if args.baseline:
                    # Initialize base optimizer (Adam)
                    if args.use_algorithm == "sam":
                        base_optimizer = Adam
                        self.optimizer = SAM(self.model.parameters(),  base_optimizer,named_params=model.named_parameters(), lr=args.learning_rate, rho=args.sam_rho, peft_weight=args.peft_weight,  random_weight = args.random_weight)
                    elif args.use_algorithm == "esam":
                        base_optimizer = Adam
                        self.optimizer = ESAM(self.model.parameters(), base_optimizer, lr=args.learning_rate, rho=args.sam_rho,beta=args.beta,gamma=args.gamma)
                    elif args.use_algorithm == "gam":
                        base_optimizer = Adam
                        self.optimizer = GAM(self.model.parameters(), base_optimizer, lr=args.learning_rate,model=self.model)
                else:
                    self.optimizer = Adam(self.model.parameters(), lr=args.learning_rate)
            elif args.optimizer == "sgd":
                #print(1)
                if args.baseline:
                    #print(2)
                    # Initialize base optimizer (SGD)
                    if args.use_algorithm == "sam":
                        #print(3)
                        base_optimizer = SGD
                        #print("model_parameters", self.model.parameters())
                        #print("model_parameters:")
                        #print(len(list(self.model.parameters())))

                        self.optimizer = SAM(self.model.parameters(), base_optimizer, lr=args.learning_rate, rho=args.sam_rho, momentum=args.momentum)
                    elif args.use_algorithm == "esam":
                        base_optimizer = SGD
                        self.optimizer = ESAM(self.model.parameters(), base_optimizer, lr=args.learning_rate, rho=args.sam_rho,beta=args.beta,gamma=args.gamma)
                else:
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                #         break
                # 第二段替换
                # if (not is_torch_version(">=", "1.11")) or (not is_random_sampler):
                #     # We just need to begin an iteration to create the randomization of the sampler.
                #     # That was before PyTorch 1.11 however...
                #     for _ in train_dataloader:
                #         break
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if args.trainer in ["zo_sgd", "zo_adam", "zo_sign_opt", "zo_conserv"]:
                        ...
                        self.gradient_update(model,inputs,targets,epoch,step)
    ...
    def save_gradients_to_csv(self, epoch, step, save_dir='gradient_logs'):
        """
        收集模型中所有参数的梯度，并将其保存到CSV文件中，使用JSON格式保存梯度值。
        """
        ...
        for name, param in self.model.named_parameters():
            if param.grad is not None:
                ...
                gradient_data.append({
                    ...
                    'Step': step,
                    'Parameter Name': name,
                    'Gradient Mean': grad.mean(),
                    'Gradient Std': grad.std(),
                    'Gradient Min': grad.min(),
                    'Gradient Max': grad.max(),
                    'Gradient Values': json.dumps(grad.tolist())  # 使用JSON格式保存
                })

        df = pd.DataFrame(gradient_data)
        file_path = os.path.join(save_dir, f'gradients_epoch_{epoch}_step_{step}.csv')
        df.to_csv(file_path, index=False)
        print(f"Gradients saved to {file_path}")
        
    def save_all_gradients_to_csv(self, epoch, step, save_dir='gradient_logs'):
        """
        保存所有可训练参数的全局梯度均值：
         - Epoch
         - Step
        ...
        # 这里的文件名可以固定为一个文件，这样只会输出一个 CSV。
        file_path = os.path.join(save_dir, "gradients.csv")

        # 收集所有梯度
        all_grads = []
        for name, param in self.model.named_parameters():
            if param.grad is not None:
                grad = param.grad.detach().cpu().view(-1)
        ...
        # 将一条记录转换为 DataFrame，以便使用 to_csv 方便地写入（或追加写入）
        df = pd.DataFrame([row_data])

        # 如果文件不存在，写入时带表头；如果文件存在，说明是追加写入，不带表头
        header = not os.path.exists(file_path)
        df.to_csv(file_path, mode='a', header=header, index=False)

        print(f"[save_gradients_to_csv] Epoch={epoch}, Step={step}, "
              f"Gradient Mean={grad_mean:.6f} saved to {file_path}")
        
    def save_top5_percent_gradient_ratio(self, epoch, step, save_dir='gradient_logs'):
        """
        计算「梯度绝对值最大的前 5% 的元素之和 / 所有梯度绝对值之和」的比值，并保存到 CSV：
        - Epoch
        - Step
        - Top5% Ratio
        """
        ...
        file_path = os.path.join(save_dir, "top1_gradients_prefix.csv")
        ...
        if len(all_grads) > 0:
            # 拼接成一个张量
            all_grads = torch.cat(all_grads)
            # 取绝对值（可以根据需求修改，比如用 all_grads**2 再开方来做 L2 等）
            abs_grads = all_grads.abs()

            # 计算所有梯度绝对值之和
            total_sum = abs_grads.sum().item()

            if total_sum > 0:
                # 计算前 5% 的大小
                k = int(len(abs_grads) * 0.01)
                # 如果 5% 计算结果为 0，则做一个最小控制
                k = max(k, 1)

                # 取出绝对值最大的前 k 个元素，并计算它们的和
                topk_values, _ = torch.topk(abs_grads, k)
                topk_sum = topk_values.sum().item()

                # 计算比值
                top5_ratio = topk_sum / total_sum
            else:
                # total_sum 为 0，说明梯度全部为 0，直接返回 0
                top5_ratio = 0.0
        else:
            # 无梯度，直接返回 0
            top5_ratio = 0.0
        ...
        row_data = {
            "Epoch": epoch,
            "Step": step,
            "Top5% Ratio": top5_ratio
        }

        # 写入 CSV
        df = pd.DataFrame([row_data])
        header = not os.path.exists(file_path)  # 如果文件不存在，写入表头
        df.to_csv(file_path, mode='a', header=header, index=False)

        print(f"[save_top5_percent_gradient_ratio] Epoch={epoch}, Step={step}, "
            f"Top5% Ratio={top5_ratio:.6f} saved to {file_path}")
    
    def gradient_update(self, model,inputs,targets,epoch,step):
        # print("epoch")
        # print(epoch)
        # print("step")
        # print(step)        
        ...
        #    else:
        #        gradients.append(np.zeros_like(param.data.detach().cpu().numpy().flatten()))
        # 将梯度展平并转换为numpy数组
        #gradients = np.concatenate(gradients)
        # 保存梯度到Excel
        #grad_df = pd.DataFrame(gradients, columns=['Gradient_Value'])
        #grad_df.to_excel('gradients.xlsx', index=False)
        ...
        if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.deepspeed:
            ...
            # 新版本推荐使用 accelerator 判断混合精度状态
            if self.do_grad_scaling and hasattr(self, "scaler") and self.scaler is not None:
                # 新版本可能要求先检查 scaler 是否存在
                # 如果使用 accelerator 则改为: if self.accelerator.scaler is not None
                if is_torch_tpu_available():
                    gradients = xm._fetch_gradients(self.optimizer)
                    xm.all_reduce("sum", gradients, scale=1.0 / xm.xrt_world_size())
                ...
                if hasattr(self, "scaler"):
                    self.scaler.unscale_(self.optimizer)
                elif hasattr(self, "accelerator") and self.accelerator.scaler is not None:
                    self.accelerator.scaler.unscale_(self.optimizer)

            # 新版本优化器处理（优先使用 accelerator）
            if is_sagemaker_mp_enabled() and args.fp16:
                self.optimizer.clip_master_grads(args.max_grad_norm)
            elif hasattr(self.optimizer, "clip_grad_norm"):
                self.optimizer.clip_grad_norm(args.max_grad_norm)
        ...
        # 在执行优化步骤之前保存梯度
        #self.save_gradients_to_csv(epoch, step)
        #self.save_all_gradients_to_csv(epoch, step)
        #self.save_top5_percent_gradient_ratio(epoch, step)
        # Optimizer step
        optimizer_was_run = True
        
        if args.baseline:  # Assuming you have a flag to determine if SAM should be used
            # SAM optimizer step
            if args.use_algorithm == "sam":
                closure = lambda: self.training_step(model, inputs)  # Replace with your actual loss function
                closure = torch.enable_grad()(closure)
                #closure()
                #print("model_parameters2:")
                #print(len(list(self.model.parameters())))
                self.optimizer.first_step(zero_grad=True)
                closure()
                #print("model_parameters3:")
                #print(len(list(self.model.parameters())))
                self.optimizer.second_step()
            if args.use_algorithm == "gam":
                closure = lambda: self.training_step(model, inputs)  # Replace with your actual loss function
                closure = torch.enable_grad()(closure)
                #closure()
                #print("model_parameters2:")
                #print(len(list(self.model.parameters())))
                self.optimizer.perturb_weights(perturb_idx=0)
                self.optimizer.disable_running_stats(self.model)
                closure()
                self.optimizer.unperturb(perturb_key="e_w_0")
                self.optimizer.grad_norm_ascent()
                closure()
                self.optimizer.perturb_weights(perturb_idx=1)
                closure()
                self.optimizer.gradient_decompose(args=self.args)
                self.optimizer.unperturb(perturb_key="e_w_1_2")
                #print("model_parameters3:")
                #print(len(list(self.model.parameters())))
            elif args.use_algorithm == "esam":
                ...
                closure = lambda: self.training_step(model, inputs)  # Replace with your actual loss function
                closure = torch.enable_grad()(closure)
                l_before = self.compute_loss(model, inputs)
                closure()
                self.optimizer.first_step(True)
                #with torch.no_grad():
                #l_after = self.training_step(model, inputs)
                l_after = self.compute_loss(model, inputs)
                ...
                if prob >=0.99:
                    ...
                    cutoff = cutoff[-1]
                    print("------enter instance_sharpness------")
                ...
                closure = lambda: self.training_step(model, inputs)
                closure()
                self.optimizer.second_step(True)
        else:        
    ...
    def do_grad_scaling(self):
        # 根据实际参数判断
        return self.args.fp16 or self.args.bf16
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        # ):
            ...
            # 检查是否是 ZERO_DP_2 或 ZERO_DP_3 模式，或者启用了 FSDP
            "zero_dp_2" in getattr(self.args, "sharded_ddp", [])  # 防御性获取属性
            or "zero_dp_3" in getattr(self.args, "sharded_ddp", [])
            or self.args.fsdp is not None
        ):
