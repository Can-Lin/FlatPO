...
from trainer2 import OurTrainer
from utils import *
...
def main():
    ...
    if args.prefix_tuning:
        ...
        args.mode = "ft"
    args.tag = f"{args.trainer}----+!{args.peft_weight}-{args.task_name}-{args.template_ver}-{args.model_name.split('/')[-1]}-OPTIM_{args.mode}-STEP{args.max_steps}-{args.optimizer}-LR{args.learning_rate}-{args.lr_scheduler_type}-ZOEPS{args.zo_eps}-Q{args.q}"
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if args.trainer != "none":
                ...
                # 保存模型
                #torch.save(framework.model.state_dict(), os.path.join(args.output_dir, 'model_state_dict_sgd.pth'))
